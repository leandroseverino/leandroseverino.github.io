(function ($) {
    ...
    $.fn.videoBackground = function(video, options) {
        ...
        settings.poster = options.poster ? options.poster : 'img/poster.jpg';

        //append a video tag to the target element
        that.append($('<video>')
            .attr({'autoplay': settings.autoplay, 'muted': settings.muted, 'loop': settings.loop, 'src': settings.src, 'poster': settings.poster})
